...
import { getAllDeliveryPlaces, createDeliveryPlace, updateDeliveryPlace, deleteDeliveryPlace } from '../services/adminService';
...
const AdminDeliveryPlaceManagementPage: React.FC = () => {
  ...
  const filterPlaces = useCallback(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(p =>
        String(p['배송지명'] || '').toLowerCase().includes(term) ||
        String(p['소속팀'] || '').toLowerCase().includes(term) ||
        String(p['주소'] || '').toLowerCase().includes(term)
      );
    }
  ...
  const handleDelete = async (placeName: string) => {
    ...
    try {
      ...
      if (result.success) {
        await loadPlaces();
        setToast({ message: result.message || '배송지가 삭제되었습니다.', type: 'success' });
      } else {
        setError(result.message || '배송지 삭제에 실패했습니다.');
        setToast({ message: result.message || '배송지 삭제에 실패했습니다.', type: 'error' });
      }
    } catch (err: any) {
        ...
        <div className="mb-6 flex justify-between items-center">
          <div>
            ...
            <p className="mt-2 text-gray-600">
              총 {filteredPlaces.length}개의 배송지가 등록되어 있습니다.
            </p>
          </div>
          <button
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={formData['배송지명']}
                    onChange={(e) => setFormData({ ...formData, '배송지명': e.target.value })}
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={formData['소속팀']}
                    onChange={(e) => setFormData({ ...formData, '소속팀': e.target.value })}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={formData['담당자']}
                      onChange={(e) => setFormData({ ...formData, '담당자': e.target.value })}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md"
                    />
                  </div>
                </div>
                <div>
                  ...
                  <select
                    value={formData['활성화']}
                    onChange={(e) => setFormData({ ...formData, '활성화': e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md"
                  >
                ...
                <div>
                  ...
                  <textarea
                    value={formData['비고']}
                    onChange={(e) => setFormData({ ...formData, '비고': e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md"
                    rows={3}
                  />
                </div>
