...
class RequestService {
  ...
  createRequest(formData, user) {
    try {
      ...
      if (Array.isArray(formData.photoUrls) && formData.photoUrls.length > 0) {
        formData.photoUrls.forEach((rawPhoto, index) => {
          const photo = String(rawPhoto || '').trim();
          if (!photo) return;

          if (photo.startsWith('data:')) {
            uploadedPhotoUrls.push(this._uploadPhoto(requestNo + '_' + (index + 1), photo));
          } else {
      ...
      // 하위 호환: 단일 photoUrl/photoBase64 처리
      if (uploadedPhotoUrls.length === 0) {
        if (formData.photoUrl && String(formData.photoUrl).startsWith('data:')) {
          photoUrl = this._uploadPhoto(requestNo, formData.photoUrl);
        ...
        // 시트는 단일 컬럼이므로 줄바꿈 구분자로 저장 (프론트에서 분리 파싱)
        photoUrl = uploadedPhotoUrls.join('\n');
      }
  ...
  _validateRequestData(data) {
    ...
    // photoUrl/photoBase64 또는 photoUrls(다중) 중 하나는 필수
    const hasMultiPhotos = Array.isArray(data.photoUrls) && data.photoUrls.length > 0;
    if (!data.photoUrl && !data.photoBase64 && !hasMultiPhotos) {
