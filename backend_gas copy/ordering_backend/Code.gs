...
function getAllUsers(sessionToken) {
  try {
    ...
    // 관리자 판별에만 사용하는 보조 함수
    // (orderingRole/auditRole 반환값은 DB 원문 그대로 유지)
    const isAdminLike_ = (v) => {
      const s = String(v === null || v === undefined ? '' : v).trim();
      if (s === '') return false;
      const lower = s.toLowerCase();
      return s === CONFIG.ROLES.ADMIN || lower.includes('관리자') || lower.includes('manager');
    };
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const orderingRoleValue = String(orderingRoleRaw === null || orderingRoleRaw === undefined ? '' : orderingRoleRaw).trim();
      const auditRoleValue = String(auditRoleRaw === null || auditRoleRaw === undefined ? '' : auditRoleRaw).trim();
      const legacyRoleValue = String(legacyRoleRaw === null || legacyRoleRaw === undefined ? '' : legacyRoleRaw).trim();
      const uiRole =
        isAdminLike_(orderingRoleValue) || isAdminLike_(auditRoleValue) || isAdminLike_(legacyRoleValue)
          ? CONFIG.ROLES.ADMIN
          : CONFIG.ROLES.USER;
      ...
      users.push({
        ...
        role: uiRole,
        // 시스템별 role: DB 원문 값을 그대로 반환 (예: 신청자/관리자/기타)
        orderingRole: orderingRoleValue,
        auditRole: auditRoleValue,
